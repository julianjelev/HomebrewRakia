...
public class SugarCorrectionActivity extends AppCompatActivity {
    ...
    private SugarCorrectionService service;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_sugar_correction);

        service = new SugarCorrectionService(this);
        ...
        txtInputTemperature.setKeyListener(DigitsKeyListener.getInstance("0123456789"));
        txtInputDegree.setKeyListener(DigitsKeyListener.getInstance("-0123456789"));
        ...
        if (callerIntent.hasExtra(Constants.EXTRA_INPUT_VALUE)) {
            ...
            if (!inputValue.equals("")) {
                int v = (int) Float.parseFloat(inputValue);
                if (v % 2 == 1)
                    v++;
                txtInputDegree.setText(String.format("%d", v));
            }
        }
    ...
    private Boolean correctDegree() {
        ...
        txtResult.setText(String.format("%.1f", correctedDegree));
        return true;
    }
}
